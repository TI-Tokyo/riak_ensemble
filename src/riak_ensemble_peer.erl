...
probe(init, State) ->
    logger:debug("~p: probe init", [State#state.id]),
...
pending(init, State) ->
    logger:debug("~p: pending init", [State#state.id]),
...
pending(pending_timeout, State) ->
    logger:debug("~p: pending_timeout", [State#state.id]),
    probe({timeout, []}, State);
pending({prepare, Id, NextEpoch, From}, State=#state{fact=Fact}) ->
    Epoch = epoch(State),
    case NextEpoch > Epoch of
        true ->
            logger:debug("~p: accepting ~p from ~p (~p)", [Id, NextEpoch, Id, Epoch]),
            reply(From, Fact, State),
            State2 = cancel_timer(State),
            prefollow({init, Id, NextEpoch}, State2);
        false ->
            logger:debug("~p: rejecting ~p from ~p (~p)", [Id, NextEpoch, Id, Epoch]),
...
pending({commit, NewFact, From}, State) ->
    ...
    case NewFact#fact.epoch >= Epoch of
        true ->
            reply(From, ok, State),
            logger:debug("~p: accepting commit from ~p for epoch ~p",
...
repair(init, State=#state{tree=Tree}) ->
    logger:debug("~p: repair", [State#state.id]),
...
prepare(init, State=#state{id=Id}) ->
    ...
    logger:debug("~p: prepare", [State#state.id]),
...
following(init, State) ->
    logger:debug("~p: Following: ~p", [State#state.id, leader(State)]),
    start_exchange(State),
...
following(exchange_failed, State) ->
    logger:debug("~p: exchange failed", [State#state.id]),
    probe(init, State);
...
following(follower_timeout, State) ->
    logger:debug("~p: follower_timeout from ~p", [State#state.id, leader(State)]),
...
step_down(Next, State=#state{lease=Lease, watchers=Watchers}) ->
    logger:debug("~p: stepping down", [State#state.id]),
...
common(tree_corrupted, From, State, StateName) ->
    gen_fsm:reply(From, ok),
    logger:debug("~p: tree_corrupted in state ~p", [State#state.id, StateName]),
    repair(init, State);
...
maybe_ping(State=#state{id=Id}) ->
    ...
    case Result of
        {ok, _State2} ->
            Result;
        {_, State2} ->
            _ = logger:info("Ping failed. Stepping down: ~p", [Id]),
            {failed, State2}
    end.
...
put_obj(Key, Obj, Seq, State=#state{id=Id, members=Members, self=Self}) ->
    ...
    case local_put(Self, Key, Obj2, ?LOCAL_PUT_TIMEOUT) of
        failed ->
            logger:warning("Failed local_put for Key ~p, Id = ~p", [Key, Id]),
            gen_fsm:sync_send_event(Self, request_failed, infinity),
            {failed, State2};
        Local ->
            case wait_for_quorum(Future) of
...
verify_hash(Key, notfound, KnownHash, _State) ->
    case KnownHash of
        notfound ->
            true;
        _ ->
            logger:warning("~p detected as corrupted", [Key]),
            false
    end;
        ...
        _ ->
            case valid_obj_hash(ObjHash, KnownHash) of
                true ->
                    true;
                false ->
                    logger:warning("~p detected as corrupted :: ~p",
...
init([Mod, Ensemble, Id, Args]) ->
    logger:debug("~p: starting peer", [Id]),
...
handle_event({watch_leader_status, Pid}, StateName, State) when node(Pid) =/= node() ->
    logger:debug("Remote pid ~p not allowed to watch_leader_status on ensemble peer ~p",
                 [Pid, State#state.id]),
    {next_state, StateName, State};
handle_event({watch_leader_status, Pid}, StateName, State = #state{watchers = Watchers}) ->
    case is_watcher(Pid, Watchers) of
        true ->
            logger:debug("Got watch_leader_status for ~p, but pid already in watchers list"),
            {next_state, StateName, State};
        false ->
...
handle_event({stop_watching, Pid}, StateName, State = #state{watchers = Watchers}) ->
    case remove_watcher(Pid, Watchers) of
        not_found ->
            logger:debug("Tried to stop watching for pid ~p, but did not find it in watcher list"),
            {next_state, StateName, State};
        {MRef, NewWatcherList} ->
...
open_hashtree(Ensemble, Id, TreeId, Path) ->
    ...
    try
        ...
        Pid
            ...
            logger:info("Failed to open hashtree: ~p/~p", [A,B]),
            timer:sleep(1000),
            open_hashtree(Ensemble, Id, TreeId, Path)
    end.
...
save_fact(#state{ensemble=Ensemble, id=Id, fact=Fact}) ->
    try
        ...
        _:Err ->
            %% logger:error("Failed saving ensemble ~p state to ~p: ~p",
